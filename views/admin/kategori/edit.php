...
<div class="content">
    <div class="page-header">
        <h2><i class="bi bi-pencil-square me-2"></i>Edit Kategori</h2>
    </div>

    <div class="form-container">
        <div class="form-card">
            <h3 class="form-title">Edit Data Kategori</h3>
            ...
            <form method="POST" action="../../../controllers/kategori.php">
                    ...
                    <select name="submenu_id" class="form-select" required>
                        ...
                        <?php foreach ($submenus as $s): ?>
                            <option value="<?= $s['id'] ?>" <?= $kategori['submenu_id'] == $s['id'] ? 'selected' : '' ?>>
                                <?= htmlspecialchars($s['nama_submenu']) ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="deskripsi" id="deskripsi" class="form-control" rows="4" required><?= htmlspecialchars($kategori['deskripsi']) ?></textarea>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        const initialWords = deskripsiInput.value.trim().split(/\s+/).filter(Boolean);
        wordCount.textContent = initialWords.length + " / 25 kata";
        if (initialWords.length >= 25) {
